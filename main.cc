...
int main(int argc, char *argv[])
{
  ...
  CAEN_DGTZ_DPP_PHA_Params_t PKU_DGTZ_DPPParams[MAXNB];//ok
  CAEN_DGTZ_BoardInfo_t PKU_DGTZ_BoardInfo;//ok
  CAEN_DGTZ_DPP_PHA_Event_t *PKU_DGTZ_Events[MaxNChannels];  // events buffer  ok
  CAEN_DGTZ_DPP_PHA_Waveforms_t *PKU_DGTZ_Waveform=NULL;     // waveforms buffer  ok
  	  ...
  	  for (ch = 0; ch < MaxNChannels; ch++) 
  	    {
  		  ...
  		  if (PKU_DGTZ_Events[ch][ev].Energy > 0)// Energy
  		    {
  		  ...
  		  if((PKU_DGTZ_Params[b].DPPAcqMode != CAEN_DGTZ_DPP_ACQ_MODE_List) && (PKU_DGTZ_RunManager.WriteFlag || flagplot))//CAEN_DGTZ_DPP_ACQ_MODE_Oscilloscope  CAEN_DGTZ_DPP_ACQ_MODE_Mixed
		      ...
		      // Use waveform data here...
  		      ...
  		      if(PKU_DGTZ_RunManager.WriteFlag)
  		      	SaveWaveform(b, ch, size,PKU_DGTZ_Events[ch][ev].Energy, WaveLine, PKU_DGTZ_RunManager.PrevTime[b][ch]);
